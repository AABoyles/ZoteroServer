...
class Zotero_Feed {
	...
	public function detectZoteroEntryType($entryNode) {
		 ...
		 */
		if ($itemTypeNodes -> length)
			return 'item';
		if ($numCollectionsNodes -> length)
			return 'collection';
		if ($numItemsNodes -> length && !($collectionKeyNodes -> length))
			return 'tag';
...
class Zotero_Entry {
	...
	public function __construct($entryNode) {
		...
		foreach ($entryNode->getElementsByTagName("link") as $linkNode) {
			if ($linkNode -> getAttribute('rel') == "enclosure") {
				$this -> links['enclosure'][$linkNode -> getAttribute('type')] = array('href' => $linkNode -> getAttribute('href'), 'title' => $linkNode -> getAttribute('title'), 'length' => $linkNode -> getAttribute('length'));
			} else {
				$this -> links[$linkNode -> getAttribute('rel')][$linkNode -> getAttribute('type')] = array('href' => $linkNode -> getAttribute('href'));
			}
		}
	}
	...
	public function getContentType($entryNode) {
		...
		if ($contentNode)
			return $contentNode -> getAttribute('type') || $contentNode -> getAttributeNS('http://zotero.org/ns/api', 'type');
		else
			return false;
	}
...
class Zotero_Cite {
	 ...
	 */
	private static $zoteroNameMap = array("author" => "author", "editor" => "editor", "bookAuthor" => "container-author", "composer" => "composer", "interviewer" => "interviewer", "recipient" => "recipient", "seriesEditor" => "collection-editor", "translator" => "translator");

	/**
	 * Mappings for text variables
	 */
	private static $zoteroFieldMap = array("title" => array("title"), "container-title" => array("publicationTitle", "reporter", "code"), /* reporter and code should move to SQL mapping tables */
	"collection-title" => array("seriesTitle", "series"), "collection-number" => array("seriesNumber"), "publisher" => array("publisher", "distributor"), /* distributor should move to SQL mapping tables */
	"publisher-place" => array("place"), "authority" => array("court"), "page" => array("pages"), "volume" => array("volume"), "issue" => array("issue"), "number-of-volumes" => array("numberOfVolumes"), "number-of-pages" => array("numPages"), "edition" => array("edition"), "version" => array("version"), "section" => array("section"), "genre" => array("type", "artworkSize"), /* artworkSize should move to SQL mapping tables, or added as a CSL variable */
	"medium" => array("medium", "system"), "archive" => array("archive"), "archive_location" => array("archiveLocation"), "event" => array("meetingName", "conferenceName"), /* these should be mapped to the same base field in SQL mapping tables */
	"event-place" => array("place"), "abstract" => array("abstractNote"), "URL" => array("url"), "DOI" => array("DOI"), "ISBN" => array("ISBN"), "call-number" => array("callNumber"), "note" => array("extra"), "number" => array("number"), "references" => array("history"), "shortTitle" => array("shortTitle"), "journalAbbreviation" => array("journalAbbreviation"), "language" => array("language"));

	private static $zoteroDateMap = array("issued" => "date", "accessed" => "accessDate");

	private static $zoteroTypeMap = array('book' => "book", 'bookSection' => "chapter", 'journalArticle' => "article-journal", 'magazineArticle' => "article-magazine", 'newspaperArticle' => "article-newspaper", 'thesis' => "thesis", 'encyclopediaArticle' => "entry-encyclopedia", 'dictionaryEntry' => "entry-dictionary", 'conferencePaper' => "paper-conference", 'letter' => "personal_communication", 'manuscript' => "manuscript", 'interview' => "interview", 'film' => "motion_picture", 'artwork' => "graphic", 'webpage' => "webpage", 'report' => "report", 'bill' => "bill", 'case' => "legal_case", 'hearing' => "bill", // ??
	...
	public static function convertItem($zoteroItem) {
		...
		// pages field exists
		$itemType = $zoteroItem -> get("itemType");
		//Zotero_ItemTypes::getName($zoteroItem->itemTypeID);
		$cslType = isset(self::$zoteroTypeMap[$itemType]) ? self::$zoteroTypeMap[$itemType] : false;
		if (!$cslType)
			$cslType = "article";
		$ignoreURL = (($zoteroItem -> get("accessDate") || $zoteroItem -> get("url")) && in_array($itemType, array("journalArticle", "newspaperArticle", "magazineArticle")) && $zoteroItem -> get("pages") && self::$citePaperJournalArticleURL);

		$cslItem = array('id' => $zoteroItem -> owningLibrary -> libraryID . "/" . $zoteroItem -> get("key"), 'type' => $cslType);
		...
		foreach ($creators as $creator) {
			...
			// isset(self::$zoteroNameMap[$creatorType]) ? self::$zoteroNameMap[$creatorType] : false;
			if (!$creatorType)
				continue;
...
class Zotero_Item extends Zotero_Entry {
	...
	public function attachmentIsSnapshot() {
		if (!isset($this -> links['enclosure']))
			return false;
		if (!isset($this -> links['enclosure']['text/html']))
			return false;
	...
	public function removeTag($rmtagname) {
		...
		foreach ($itemTags as $ind => $tag) {
			if ((is_string($tag) && $tag == $rmtagname) || (is_array($tag) && isset($tag['tag']) && $tag['tag'] == $rmtagname)) {
...
class Zotero_Group extends Zotero_Entry {
	...
	public function updateString() {
		...
		// str_replace('&#039;', '&apos;', htmlspecialchars($this->name, ENT_QUOTES)));
...
class Zotero_Library {
	...
	public function apiQueryString($passedParams = array()) {
		...
		$queryParamOptions = array('start', 'limit', 'order', 'sort', 'content', 'q', 'itemType', 'locale', 'key', 'itemKey', 'tag', 'tagType', 'style', 'format', 'linkMode', 'linkwrap');
	...
	public function uploadNewAttachedFile($item, $fileContents, $fileinfo = array()) {
		...
		if ($response -> getStatus() == 200) {
			...
			if (!empty($resObject['exists'])) {
				...
				return true;
				//api already has a copy, short-circuit with positive result
			} else {
	...
	public function getCV($userID = '') {
		...
		foreach ($sectionNodes as $sectionNode) {
			...
			$c = $doc -> saveHTML($sectionNode);
			// $sectionNode->nodeValue;

...
class HTTP_WebDAV_Server {
	...
	function ServeRequest() {
		...
		if ((!(($_SERVER['REQUEST_METHOD'] == 'OPTIONS') && ($this -> path == "/"))) && (!$this -> _check_auth())) {
	...
	function _copymove($what) {
		...
		if ($http_host == $http_header_host && !strncmp($_SERVER["SCRIPT_NAME"], $path, strlen($_SERVER["SCRIPT_NAME"]))) {
	...
	function mkprop() {
		$args = func_get_args();
		if (count($args) == 3) {
			return array("ns" => $args[0], "name" => $args[1], "val" => $args[2]);
		} else {
			return array("ns" => "DAV:", "name" => $args[0], "val" => $args[1]);
		}
	}
	...
	function _check_auth() {
		if (method_exists($this, "checkAuth")) {
			...
			return $this -> checkAuth(@$_SERVER["AUTH_TYPE"], @$_SERVER["PHP_AUTH_USER"], @$_SERVER["PHP_AUTH_PW"]);
	...
	function _urlencode($url) {
		return strtr($url, array(" " => "%20", "&" => "%26", "<" => "%3C", ">" => "%3E", ));
	}
